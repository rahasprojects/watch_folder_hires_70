...
class UploadPanel51(ttk.Frame):
    ...
    def _insert_queue_row(self, job: UploadJob, status_type: str, position: int = 0):
        ...
        if status_type == 'active':
            self.tree.tag_configure('active', background='#fff3e0')  # Oranye muda
    ...
    def _create_job_progress(self, job: UploadJob):
        """Buat progress bar untuk job baru"""
        ...
        name_label = ttk.Label(header_frame, text=f"⭐ {job.file_name}", font=('Arial', 9, 'bold'))
        name_label.pack(side='left')
        
        # Speed label
        speed_label = ttk.Label(header_frame, text="", font=('Arial', 8), foreground='#f39c12')
        ...
        eta_label = ttk.Label(info_frame, text="", font=('Arial', 8), foreground='#f39c12')
    ...
    def _update_job_progress(self, job: UploadJob):
        ...
        if job.speed_mbps > 0:
            ...
            if job.speed_mbps > 30:
                widgets['speed_label'].config(text=f"⚡ {speed_text}", foreground='#f39c12')
            elif job.speed_mbps > 10:
                widgets['speed_label'].config(text=f"📊 {speed_text}", foreground='#3498db')
            else:
                widgets['speed_label'].config(text=f"🐢 {speed_text}", foreground='#e67e22')
        else:
