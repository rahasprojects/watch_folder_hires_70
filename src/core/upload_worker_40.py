# -*- coding: utf-8 -*-
"""
Worker thread untuk upload file ke LOWRES (40) - PRIORITAS NORMAL
"""
...
class UploadWorker40(threading.Thread):
    """
    Worker thread untuk menangani upload satu file ke server 40 (LOWRES)
    PRIORITAS: NORMAL
    """
    ...
    def __init__(self, worker_id: int, queue_manager, upload_manager, 
        ...
        logger.info(f"UploadWorker40-{worker_id} initialized (NORMAL PRIORITY)")
    
    def run(self):
        """Main loop worker"""
        logger.info(f"UploadWorker40-{self.worker_id} started")
        
        while self.running:
            try:
                # Ambil job berikutnya dari queue 40 (NORMAL PRIORITY)
                job = self.queue_manager.get_next_job_40()
                ...
                logger.error(f"UploadWorker40-{self.worker_id} error: {e}")
                time.sleep(5)
        
        logger.info(f"UploadWorker40-{self.worker_id} stopped")
    ...
    def _process_upload(self, job: UploadJob):
        ...
        """
        logger.info(f"Worker40-{self.worker_id} processing: {job.file_name} (NORMAL PRIORITY)")
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"Worker40-{self.worker_id} completed: {actual_filename} in {duration:.2f}s")
            ...
            logger.error(f"Worker40-{self.worker_id} failed: {job.file_name} - {e}")
            ...
            if job.retry_count < job.max_retry - 1:
                ...
                logger.info(f"Worker40-{self.worker_id} will retry {job.file_name} ({job.retry_count + 1}/{job.max_retry})")
                self.queue_manager.fail_job(job, str(e), retry=True)
            else:
                # Gagal permanen
                logger.error(f"Worker40-{self.worker_id} permanent failure: {job.file_name}")
    ...
    def stop(self):
        """Hentikan worker"""
        self.running = False
        logger.info(f"UploadWorker40-{self.worker_id} stopping...")
...
if __name__ == "__main__":
    ...
    print("UploadWorker40 class ready (NORMAL PRIORITY)")
